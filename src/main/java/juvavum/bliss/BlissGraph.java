 ...
 * 
 * <ul>
 * <li>renamed to BlissGraph</li>
 * <li>fixed some generics related issues</li>
 * <li>removed reporter</li>
 * </ul>
 * 
 * @author Tommi Junttila
 * @author Martin Schneider
 */
public class BlissGraph<V extends Comparable<V>> implements
		Comparable<BlissGraph<V>> {
	/*
	 * Intermediate translator stuff for mapping bliss automorphisms back to
	 * jbliss automorphisms
	 */
	 ...
	 *            the vertex indentifier
	 * @param color
	 *            the color of the vertex (a non-negative integer)
	 * @return true if the vertex was not already in the graph
	 */
	...
	/**
	 * Add an undirected edge between the vertices v1 and v2. If either of the
	 * vertices is not in the graph, it will be added. Duplicate edges between
	...
	/**
	 * Find (a generating set for) the automorphism group of the graph. If the
	 * argument reporter is non-null, then a generating set of automorphisms is
	 * reported by calling its {@link Reporter#report} method for each
	 * generator.
	 * 
	 * @param reporter
	 *            An object implementing the Reporter interface
	 * @param reporter_param
	 *            The parameter passed to the Reporter object
	 */
	public void find_automorphisms(Reporter reporter, Object reporter_param) {
		...
		for (Map.Entry<V, Vertex> e : vertices.entrySet()) {
			...
			for (Vertex vertex2 : vertex.edges) {
				if (v.compareTo(vertex2.id) <= 0)
					_add_edge(bliss, _bliss_map.get(vertex.id),
	...
	/**
	 * Find the canonical labeling and the automorphism group of the graph. If
	 * the argument reporter is non-null, then a generating set of automorphisms
	 * is reported by calling its {@link Reporter#report} method for each
	 * generator.
	 * 
	...
	/**
	 * Copy and relabel the graph. The labeling is a Map that associates each
	 * vertex in the graph into new vertex.
	 * 
	 * @param labeling
	 *            the labeling to apply
	...
	static {
		/*
		 * Load the C++ library including the true bliss and the JNI interface
		 * code
		 */
